...
void loop () {
  ...
  if (digitalRead(A3) && digitalRead(A2) && digitalRead(A1)&& digitalRead(A0)) {
  lcd.print("Crowded!          ");
  } else if (digitalRead(A3) || digitalRead(A2) || digitalRead(A1) || digitalRead(A0)) {
    lcd.print(!digitalRead(A3) + !digitalRead(A2) + !digitalRead(A1) + !digitalRead(A0));
    lcd.print(" Available   ");
  } else {
    lcd.print("All Available");
  }
}

https://www.tinkercad.com/things/7PtWTfGv0nq-estacionamento-1
